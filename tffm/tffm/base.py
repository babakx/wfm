...
def batcher_bpr(X_, X_neg, y_=None, batch_size=-1):
    n_samples = X_.shape[0]
    ...
    for i in range(0, n_samples, batch_size):
        upper_bound = min(i + batch_size, n_samples)
        ret_x = X_[i:upper_bound]
        ret_x_neg = X_neg[i:upper_bound]
        ret_y = None
        if y_ is not None:
            ret_y = y_[i:i + batch_size]
        yield (ret_x, ret_x_neg, ret_y)
...
def batch_to_feeddict_bpr(X, X_neg, core, y=None):
    fd = {}

    # sparse case
    X_sparse = X.tocoo()
    X_sparse_neg = X_neg.tocoo()
    ...
    fd[core.raw_indices_neg] = np.hstack(
        (X_sparse_neg.row[:, np.newaxis], X_sparse_neg.col[:, np.newaxis])
    ).astype(np.int64)
    fd[core.raw_values_neg] = X_sparse_neg.data.astype(np.float32)
    fd[core.raw_shape_neg] = np.array(X_sparse_neg.shape).astype(np.int64)
...
class TFFMBaseModel(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def fit_bpr(self, X_, X_neg, n_epochs=None, show_progress=False):
        if self.core.n_features is None:
            self.core.set_num_features(X_.shape[1])
        ...
        for epoch in tqdm(range(n_epochs), unit='epoch', disable=(not show_progress)):
            ...
            epoch_loss = []
            # iterate over batches
            for bX, bX_neg, _ in batcher_bpr(X_[perm], X_neg[perm], batch_size=self.batch_size):
                fd = batch_to_feeddict_bpr(bX, bX_neg, core=self.core)

                ops_to_run = [self.core.trainer, self.core.target, self.core.summary_op]
                if self.core.norm_alpha is True:
                    ops_to_run.append(self.core.normalized_alpha_op)

                result = self.session.run(ops_to_run, feed_dict=fd)
                if self.core.norm_alpha is True:
                    _, batch_target_value, summary_str, _ = result
                else:
                    _, batch_target_value, summary_str = result
